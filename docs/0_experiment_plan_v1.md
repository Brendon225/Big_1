...
| 2.3 B2: SDP | 原始依存图上的最短依存路径线性化拼接 | B2 结果 | 2天 |
| 2.4 实体折叠早期验证 | 在相同训练配置下，比较 Raw View 与 Coarsened View 的结构注入效果，包括 B1-coarse 与 B2-coarse 两组轻量对照 | 初步的实体折叠趋势结果 | 2天 |
| 2.5 结果分析 | 对比 B0、B1-raw、B2-raw、B1-coarse、B2-coarse，分别从“结构是否有用”“路径是否优于整树”“实体折叠是否初步有益”三个角度进行分析，初步回答 RQ1，并对 RQ2 给出趋势性观察 | 分析报告 | 1天 |
| 2.5 代码重构 | 整理代码结构，确保 dep_view 与 dep_form 可开关控制，为阶段三和后续正式消融实验提供统一接口 | 清晰的代码框架 | 2天 |

阶段二里程碑：B0、B1-raw、B2-raw、B1-coarse、B2-coarse 跑通，初步验证依存信息对生成式 BioRE 的有效性，并获得实体折叠是否值得在后续完整模型中继续强化的早期信号。这一步非常关键——如果 B1-raw / B2-raw 相比 B0 没有提升，需要重新审视依存信息的注入方式；如果 B1-coarse / B2-coarse 相比对应的 Raw 版本没有改善，则需要重新审视实体折叠策略与线性化接口设计。

阶段二说明：本阶段中的实体折叠实验属于早期验证（early probe），目的在于以最轻量、最可控的方式观察图粗化是否能够提升结构注入质量，而不是在此阶段对 RQ2 做最终定论。RQ2 的正式结论仍以后续完整模型上的消融实验、实体长度分桶分析与 case study 为准。
...
| 4.2 消融实验 | A1-A5 五组消融，其中 A1 用于正式验证 Entity Coarsening 的贡献 | 消融结果表 | 3天 |
| 4.3 beta 敏感性 | 8 个 beta 值 x 3 个数据集 | 敏感性曲线图 | 2天 |
| 4.4 统计检验 | 配对 t 检验 | 显著性结果 | 1天 |

阶段四里程碑：所有定量实验完成，主实验表和消融表填满。与阶段二的趋势性结果不同，本阶段的 A1-A5 消融实验将作为各模块贡献的正式定量证据，用于回答 RQ2-RQ4
...
| 5.2 Case Study | 选取典型样例，可视化分析，包括实体折叠前后依存图与线性化结构的对比 | 可视化图表 | 2天 |
